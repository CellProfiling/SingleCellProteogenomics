...
class ProteinGaussianClustering:
    ...
    def zero_center_fucci(self):
        ...
        green_fucci, red_fucci, u_plate, well_plate, plate = protein_data.green_fucci, protein_data.red_fucci, protein_data.u_plate, protein_data.well_plate, protein_data.plate
        ...
        self.log_green_fucci, self.log_red_fucci = log_green_fucci, log_red_fucci
        self.log_green_fucci_zeroc, self.log_red_fucci_zeroc = log_green_fucci_zeroc, log_red_fucci_zeroc
        self.log_green_fucci_zeroc_rescale, self.log_red_fucci_zeroc_rescale = log_green_fucci_zeroc_rescale, log_red_fucci_zeroc_rescale
    ...
    def fucci_plots(self):
        ...
        plt.hist2d(self.log_green_fucci_zeroc_rescale, self.log_red_fucci_zeroc_rescale, bins=200)
    ...
    def gaussian_clustering(self, clusternames):
        ...
        cluster_labels = gaussian.fit_predict(np.array([self.log_green_fucci_zeroc_rescale, self.log_red_fucci_zeroc_rescale]).T)
    ...
    def make_cluster_labels(self, g1_idx=1, sph_idx=2, g2_idx=0):
        ...
        cluster_labels = self.gaussian_clustering(
            clusternames
        )
    ...
    def gaussian_clustering_analysis(self, alpha_gauss=0.05, doGenerateBoxplotsPerGene=False):
        ...
        wp_ensg, well_plate, u_well_plates, ab_cell, ab_nuc, ab_cyto, mt_cell, wp_iscell, wp_isnuc, wp_iscyto = (
            self.protein_data.wp_ensg, self.protein_data.well_plate, self.protein_data.u_well_plates, 
            self.protein_data.ab_cell, self.protein_data.ab_nuc, self.protein_data.ab_cyto, self.protein_data.mt_cell, 
            self.protein_data.wp_iscell, self.protein_data.wp_isnuc, self.protein_data.wp_iscyto)
        ...
        self.wp_comp_kruskal_gaussccd_adj, self.wp_pass_kruskal_gaussccd_bh_comp, self.wp_mt_kruskal_gaussccd_adj, self.wp_pass_gaussccd_bh_mt = (
            wp_comp_kruskal_gaussccd_adj, wp_pass_kruskal_gaussccd_bh_comp, wp_mt_kruskal_gaussccd_adj, wp_pass_gaussccd_bh_mt)

    def address_replicates(self, alpha_gauss=0.05):
        '''Look for replicated protein samples and antibody stainings'''
        wp_pass_kruskal_gaussccd_bh_comp, wp_ensg, wp_ab, u_well_plates = (
            self.wp_pass_kruskal_gaussccd_bh_comp, self.protein_data.wp_ensg, self.protein_data.wp_ab, self.protein_data.u_well_plates)
