...
class ProteinDataPreparation():
    ...
    def read_raw_data(self):
        ...
        self.my_df = pd.concat((my_df1, my_df2), sort=True)
    ...
    def read_sample_info(self, df):
        ...
        self.u_plate = np.unique(plate)
        self.well_plate = np.asarray(df.well_plate)
        self.imgnb = np.asarray(df.ImageNumber)
        self.well_plate_imgnb = np.asarray([f"{wp}_{imgnb[i]}" for i,wp in enumerate(well_plate)])
        self.u_well_plates = np.unique(well_plate)
        ...
        self.ENSG = np.asarray([ensg_dict[wp] if wp in ensg_dict else "" for wp in well_plate])
        self.antibody = np.asarray([ab_dict[wp] if wp in ab_dict else "" for wp in well_plate])
        self.result = np.asarray([result_dict[wp] if wp in result_dict else "" for wp in well_plate])
        self.compartment = np.asarray([compartment_dict[wp] if wp in compartment_dict else "" for wp in well_plate])
    ...
    def previous_results(u_well_plates, result_dict, ensg_dict, ab_dict):
        ...
        self.wp_ensg = np.asarray([ensg_dict[wp] if wp in ensg_dict else "" for wp in u_well_plates])
        self.wp_ab = np.asarray([ab_dict[wp] if wp in ab_dict else "" for wp in u_well_plates])
        self.wp_prev_ccd = np.asarray([wp in result_dict and result_dict[wp].startswith("ccd") for wp in u_well_plates])
        self.wp_prev_notccd = np.asarray([wp in result_dict and result_dict[wp].startswith("notccd") for wp in u_well_plates])
        self.wp_prev_negative = np.asarray([wp in result_dict and result_dict[wp].startswith("negative") for wp in u_well_plates])
        self.prev_ccd_ensg = wp_ensg[wp_prev_ccd]
        self.prev_notccd_ensg = wp_ensg[wp_prev_notccd]
        self.prev_negative_ensg = wp_ensg[wp_prev_negative]
