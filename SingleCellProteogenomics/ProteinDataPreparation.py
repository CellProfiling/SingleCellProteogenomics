...
class ProteinData:
    ...
    def read_sample_info(self, df):
        ...
        self.well_plate_imgnb = np.asarray([f"{wp}_{self.imgnb[i]}" for i,wp in enumerate(self.well_plate)])
        self.u_well_plates = np.unique(self.well_plate)
        self.ab_objnum = np.asarray(df.ObjectNumber)
        self.well_plate_imgnb_objnb = np.asarray([f"{wp}_{self.imgnb[i]}_{self.ab_objnum[i]}" for i,wp in enumerate(self.well_plate)])
        ...
        self.result_dict = dict([(wppp[i], rrrr[i]) for i in range(len(wppp1))])
        self.compartment_dict = dict([(wppp[i], cccc[i]) for i in range(len(wppp))])
        self.ENSG = np.asarray([self.ensg_dict[wp] if wp in self.ensg_dict else "" for wp in self.well_plate])
        self.antibody = np.asarray([self.ab_dict[wp] if wp in self.ab_dict else "" for wp in self.well_plate])
        self.result = np.asarray([self.result_dict[wp] if wp in self.result_dict else "" for wp in self.well_plate])
        self.compartment = np.asarray([self.compartment_dict[wp] if wp in self.compartment_dict else "" for wp in self.well_plate])
        ...
        utils.np_save_overwriting("output/pickles/u_well_plates.npy", self.u_well_plates)
        ...
        utils.np_save_overwriting("output/pickles/well_plate_imgnb.npy", self.well_plate_imgnb)
        utils.np_save_overwriting("output/pickles/well_plate_imgnb_objnb.npy", self.well_plate_imgnb_objnb)
    
    def previous_results(self):
        '''Process the results metadata into lists of previously annotated CCD proteins'''
        u_well_plates, result_dict, ensg_dict, ab_dict = self.u_well_plates, self.result_dict, self.ensg_dict, self.ab_dict
        ...
        self.wp_ensg, self.wp_ab = wp_ensg, wp_ab 
        self.wp_prev_ccd, self.wp_prev_notccd, self.wp_prev_negative = wp_prev_ccd, wp_prev_notccd, wp_prev_negative, 
        self.prev_ccd_ensg, self.prev_notccd_ensg, self.prev_negative_ensg = prev_ccd_ensg, prev_notccd_ensg, prev_negative_ensg

    def apply_manual_filtering(self):
        '''Filter raw data based on manual annotations'''
        my_df, result_dict, ab_dict = self.raw_protein_data, self.result_dict, self.ab_dict
    ...
    def apply_cell_count_filter(self):
        ...
        print(f"{len(self.raw_protein_data)}: number of cells before filtering out samples with < {MIN_CELL_COUNT} cells")
    ...
    def apply_variation_filter(self, output_dataframes=False):
        ...
        self.my_df_filtered_variation = my_df_filtered[variable_firstbatch | variable_secondbatch]
        self.my_df_filtered_novariation = my_df_filtered[~(variable_firstbatch | variable_secondbatch)]
        print(f"{len(unfiltered_df)}: number of cells before filtering for variation")
        print(f"{len(self.my_df_filtered_variation)}: number of cells in samples with variation")
        print(f"{len(self.my_df_filtered_novariation)}: number of cells in samples without variation")
        
        if output_dataframes:
            '''output these dataframes (used for skewness / kurtosis analysis)'''
            self.my_df_filtered_variation.to_csv("output/nuc_predicted_prob_phases_filtered_variation.csv")
            self.my_df_filtered_novariation.to_csv("output/nuc_predicted_prob_phases_filtered_novariation.csv")
    ...
    def read_sample_data(self, df):
        ...
        self.ab_nuc, self.ab_cyto, self.ab_cell, self.mt_cell, self.green_fucci, self.red_fucci = ab_nuc, ab_cyto, ab_cell, mt_cell, green_fucci, red_fucci
