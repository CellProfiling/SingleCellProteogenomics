...
def getGeneNameDict():
    ...
    gene_info = pd.read_csv("input/ProteinProperties/IdsToNames.csv", index_col=False, header=None, names=["gene_id", "name", "biotype", "description"])
...
def save_gene_names_by_category(adata, wp_ensg, ccd_comp, nonccd_comp, ccdtranscript):
    ...
    bioccd = np.genfromtxt("input/processed/manual/BiologicallyDefinedCCD.txt", dtype='str') # from mitotic structures
