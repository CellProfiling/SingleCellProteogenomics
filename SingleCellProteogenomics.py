...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Perform plotting and side analyses not needed for HPA releases.",
    )
    parser.add_argument(
        "--outputFilteredDataframe",
        action="store_true",
        help="Output the protein imaging post-processing data after filtering artifacts and such.",
    )
    parser.add_argument(
        "--useLogProteinIntensities",
        action="store_true",
        help="Toggle for using log-transformed intensities. We decided to use natural intensities instead.",
    )
    parser.add_argument(
        "--keepOutlierIntensities",
        action="store_true",
        help="Keep the intensities that are 5 SD from the mean; we decided to remove these.",
    )
    ...
    protein_clustering = ProteinGaussianClustering.ProteinGaussianClustering(
        protein_data
    )
    ...
    protein_ccd = ProteinCellCycleDependence.ProteinCellCycleDependence(
        protein_data, protein_clustering, do_plotting
    )
    ...
    protein_ccd.protein_variability.calculate_variation(
        args.threads, protein_ccd, do_plotting, args.useLogProteinIntensities
    )
    ...
    # protein_ccd.calculate_cell_cycle_dependence_protein(
    #     args.threads,
    #     ALPHA_PROTEIN_FUCCI,
    #     args.useLogProteinIntensities,
    #     args.keepOutlierIntensities,
    # )
    ...
    #     ProteinCellCycleDependenceAnalysis.global_plots_protein(
    #         protein_ccd, ALPHA_PROTEIN_FUCCI
    #     )
