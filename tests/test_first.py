...
from SingleCellProteogenomics import (
    ProteinGaussianClustering, ProteinDataPreparation
    )
...
class TestTest(unittest.TestCase):
    ...
    def test_zero_centering(self):
        ...
        for plate in self.protein.protein_data.u_plate:
            ...
            greenplatemedian=np.log10(np.median(self.protein.protein_data.green_fucci[fromplate]))
            redplatemedian=np.log10(np.median(self.protein.protein_data.red_fucci[fromplate]))
            assert all(np.log10(self.protein.protein_data.green_fucci[fromplate]) == self.protein.log_green_fucci_zeroc[fromplate] + greenplatemedian)
            assert all(np.log10(self.protein.protein_data.red_fucci[fromplate]) == self.protein.log_red_fucci_zeroc[fromplate] + redplatemedian)
